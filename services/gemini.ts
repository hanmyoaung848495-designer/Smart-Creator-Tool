...
export const refineScript = async (script: string, apiKey?: string): Promise<string> => {
  const ai = getAIClient(apiKey);
  const response = await generateWithFallback(ai, 'gemini-3.1-pro-preview', {
    contents: `Refine and improve the following narrator style script to make it more engaging, professional, and attractive: \n\n${script}`
  });
...
5.Language: Provide the script in the original language of the video. 
  ${translateToBurmese ? "CRITICAL: The user has requested a translation. Please translate the entire generated script into Burmese while maintaining its style and  tone." :""}
...
export const writeScript = async (topic: string, style: string, length: string, lang: string, apiKey?: string): Promise<string> => {
  ...
  // Script ရဲ့ Quality နဲ့ Style ကို ပိုမိုကောင်းမွန်စေမယ့် Detailed Prompt
  const prompt = `Write a high-quality, professional video script about "${topic}" in the language: ${lang}.
    ...
    3. Speaker Labels: Clearly label speakers like "Name (Narrator):" or "Speaker:".
